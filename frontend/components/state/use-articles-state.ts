...
import type { Article, ArticleTag, Feed } from "./rss-types";
...
export function useArticlesState(
  ...
  const fetchArticles = async (url: string, feedInfo: Feed) => {
    ...
    try {
      ...
      if (data && data.items) {
        const parsedArticles = data.items.map((item: any) => {
          // Generate stable ID for article
          const articleId =
            item.guid ||
            item.id ||
          ...
          return {
            ...
            feedId: feedInfo?.id || "",
            tags: [], // Tags will be added through other means
            isRead: isRead,
          };
        });
  ...
  const addTagToArticle = (
    articleId: string,
    tag: Omit<ArticleTag, "article_id" | "attachedAt">
  ) => {
  ...
  const removeTagFromArticle = (articleId: string, tagId: string) => {
    // 更新全局标签列表
    setTags((prev) =>
      prev.filter((tag) => !(tag.id === tagId && tag.article_id === articleId))
    );
